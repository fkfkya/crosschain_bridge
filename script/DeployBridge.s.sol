...
contract DeployBridgeScript is Script {
    function run() external {
        ...
        string memory deploymentInfo = string(abi.encodePacked(
            ...
            "Bridge Address: ", vm.toString(address(bridge)), "\n",
            "Deployer Address: ", vm.toString(deployer), "\n",
            "Validator Address: ", vm.toString(validator), "\n",
            "Token Name: ", tokenName, "\n",
            "Token Symbol: ", tokenSymbol, "\n",
            "Initial Supply: ", vm.toString(initialSupply / 1e18), " BRT\n",
            "Max Supply: 1000000 BRT\n",
            "Min Amount: ", vm.toString(bridgeMinAmount / 1e18), " BRT\n",
            "Max Amount: ", vm.toString(bridgeMaxAmount / 1e18), " BRT\n",
            "Fee: ", vm.toString(feeBasisPoints), " basis points\n",
        ...
        console.log("3. To lock tokens: cast send <bridge_address> 'lockTokens(uint256,uint256)' <amount> <target_chain_id> --private-key <your_key>");
        console.log("4. To unlock tokens: cast send <bridge_address> 'unlockTokens(address,uint256,uint256,uint256,bytes)' <user> <amount> <nonce> <source_chain_id> <signature> --private-key <validator_key>");
