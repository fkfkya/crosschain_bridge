...
contract CrossChainBridge is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    constructor(address _token, address _validator, uint256 _minAmount, uint256 _maxAmount, uint256 _feeBasisPoints)
        Ownable(msg.sender)
    {
     ...
     */
    function lockTokens(uint256 amount, uint256 targetChainId) external whenNotPaused nonReentrant {
        ...
        uint256 nonce =
            uint256(keccak256(abi.encodePacked(msg.sender, amount, targetChainId, block.timestamp, block.number)));
        ...
        bytes32 transferHash = keccak256(abi.encodePacked(msg.sender, amount, nonce, CURRENT_CHAIN_ID, targetChainId));

        // Эмитируем событие
        emit TokensLocked(msg.sender, amount, nonce, CURRENT_CHAIN_ID, targetChainId, transferHash);
    }
     ...
     */
    function unlockTokens(address user, uint256 amount, uint256 nonce, uint256 sourceChainId, bytes memory signature)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        // Создаем хеш перевода
        bytes32 transferHash = keccak256(abi.encodePacked(user, amount, nonce, sourceChainId, CURRENT_CHAIN_ID));

        // Выпускаем токены получателю
        TOKEN.mint(user, amount);

        // Эмитируем событие
        emit TokensUnlocked(user, amount, nonce, sourceChainId, CURRENT_CHAIN_ID, transferHash);
    }
    ...
    function _verifySignature(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(user, amount, nonce, sourceChainId, CURRENT_CHAIN_ID));
    ...
    {
        return (address(TOKEN), validator, minAmount, maxAmount, feeBasisPoints, CURRENT_CHAIN_ID);
    }
     ...
     */
    function getMessageHash(address user, uint256 amount, uint256 nonce, uint256 sourceChainId)
        external
        view
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(user, amount, nonce, sourceChainId, CURRENT_CHAIN_ID));
    }
}
